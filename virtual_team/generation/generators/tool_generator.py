"""工具生成器 — 自然语言描述 → Python/JavaScript 工具代码."""
...
''',
    ...
    "http_request": {
        "keywords": ["http", "请求", "request", "api", "fetch"],
        "name": "http_request",
...
''',
        "parameters": {"json_string": {"type": "string", "required": True}},
    },
    "get_weather": {
...
JS_TOOLS: dict[str, dict[str, Any]] = {
    "read_file": {
        "keywords": ["读取", "read", "文件", "file"],
        "name": "readFile",
        "description": "读取文件内容",
...
class ToolGenerator(BaseGenerator):
    """自然语言描述 → Python / JavaScript 工具代码."""

    def generate(self, request: GenerateRequest) -> GenerateResponse:
        tool_id = f"tool_{hashlib.md5(request.description.encode(), usedforsecurity=False).hexdigest()[:8]}"  # nosec
        language = self._resolve_language(request.description, request.context)
        return self._generate_code(tool_id, request.description, language)

    def _resolve_language(self, description: str, context: dict[str, Any]) -> str:
        explicit = context.get("language", "").lower()
        if explicit in ("python", "javascript", "typescript"):
            return explicit if explicit != "typescript" else "javascript"

        desc_lower = description.lower()
        if any(w in desc_lower for w in ("javascript", "node", "js", "typescript", "ts")):
            return "javascript"
        return "python"

    def _generate_code(self, tool_id: str, description: str, language: str) -> GenerateResponse:
        if language == "python":
            return self._generate_python(tool_id, description)
        return self._generate_javascript(tool_id, description)

    def _generate_python(self, tool_id: str, description: str) -> GenerateResponse:
        template = self._match_template(description, PYTHON_TOOLS)
        ...
        return GenerateResponse(
            ...
            content=self._custom_python_code(description),
            metadata={
                "language": "python",
    ...
    def _generate_javascript(self, tool_id: str, description: str) -> GenerateResponse:
        template = self._match_template(description, JS_TOOLS)
        ...
        return GenerateResponse(
            ...
            name="customTool",
            description=description[:50],
            content=self._custom_js_code(description),
            metadata={
                "language": "javascript",
                "parameters": {"inputData": {"type": "any", "required": False}},
                "is_valid": True,
            },
        )

    @staticmethod
    def _match_template(description: str, templates: dict[str, dict[str, Any]]) -> dict[str, Any] | None:
        desc_lower = description.lower()
    ...
    @staticmethod
    def _custom_python_code(description: str) -> str:
...
'''
    ...
    @staticmethod
    def _custom_js_code(description: str) -> str:
        return f'''async function customTool(inputData = null) {{
    /**
     * {description}
...
'''
    ...
    def supported_keywords(self) -> list[str]:
        return [kw for t in PYTHON_TOOLS.values() for kw in t["keywords"]]
