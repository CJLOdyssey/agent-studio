...
async def get_skills() -> list[dict]:
    ...
    async with factory() as session:
        ...
        return [{ "id": s.id, "name": s.name, "category": s.category, "description": s.description, "version": s.version, "status": s.status, "author": s.author, "instructions": s.instructions, "prompt_id": s.prompt_id, "tool_names": s.tool_names or [], "output_constraint": s.output_constraint, "created_at": s.created_at.isoformat() if s.created_at else None, } for s in result.scalars().all()]
