"""流式发射器——将 httpx 原始流式事件桥接到 Redis pub/sub 与数据库。"""
...
class StreamEmitter:
    ...
    def _checked_append(self, buffer: list[str], item: str, label: str) -> None:
        buffer.append(item)
        # 按累计字符数而非 chunk 数限制——模型以 1-2 字符为粒度流式输出，
        # 若按 1000 个 chunk 上限会截掉长消息开头。仅在总字符数超限时才丢最旧 chunk。
        buf_id = id(buffer)
    ...
    async def __call__(self, event: dict[str, Any]) -> None:
        ...
        # on_tool_start / on_tool_end 在图层面通过 on_custom_thinking 处理
        # （见 graph.py 的 _tools_node）
    ...
    async def _flush_buffers(self) -> None:
        ...
        # 将上一次 tools-only flush 遗留的 pending thinking 与当前 thinking 合并
        ...
        if thinking_text:
            ...
            # 仅思考无正文的 flush：把 thinking 缓存为 pending，
            # 待下次带 content 的 flush 时一并写入数据库。
