...
async def _load_attachment_payload(run_id: str) -> tuple[str, str]:
    """加载 run 绑定的附件为（模型上下文、下载链接）。

    与 agent 流水线一致：extracted_text 进入模型上下文（由节点工厂以
    SystemMessage 注入），确定性下载链接追加到最终团队汇总。Fail-open——
    任何错误都降级为空载荷，使团队 run 仍能执行。
    """
    try:
...
def _resolve_final_and_roles(
    ...
    最终交付物优先为 reporter 的 ``_final_report``（若存在），否则取最后一个
    非评审节点的产物（评审输出是机器裁决，绝不是面向用户的交付物），
    再否则取最后一条消息。

    返回 (final, reviewer_roles, reporter_roles, from_role_artifact)：
    当 ``final`` 重复了某个已作为独立消息持久化的角色产物时，
    ``from_role_artifact`` 为 True——调用方必须跳过额外的「团队汇总」保存，
    以避免重载时产生重复行。
    """
