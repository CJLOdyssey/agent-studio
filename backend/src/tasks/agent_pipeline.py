"""单 agent 流水线——工具发现、RAG 上下文与图执行。"""

# ruff: noqa: E402 — tracemalloc 初始化之后的 import 是有意为之
import asyncio
import contextlib
import gc
import os
...
_run_counter = 0
_AGENT_TIMEOUT = int(os.environ.get("AGENT_TIMEOUT", "600"))  # 默认 10 分钟


async def _notify_session_changed(user_id: str, session_id: str | None) -> None:
    """向其他客户端广播会话更新（跨客户端同步）。Fail-open。"""
...
def _kill_stuck_child_processes() -> None:
    ...
    ``asyncio.timeout`` 取消协程但**不会**杀掉库所 spawn 的 OS 子进程
    （例如 LangGraph 内部的 multiprocessing fork）。这些孤儿进程会无限
    占用 CPU。
    """
    try:
        ppid = os.getpid()
...
async def _run_agent_pipeline(
    ...
    # 工具由前端 API 创建——无硬编码的默认工具
    # 通过共享构建器绑定 agent 的工具 / MCP / 技能（团队工作流引擎也使用，
    # 因此两条路径产生相同的 ToolConfig）。
    if agent_id and ac:
    ...
    # ── 意图检测：对「打开XX」模式直接打开 URL ──
    from tasks.intent import detect_open_url

    _open_url = detect_open_url(requirement)
    if _open_url:
    ...
    try:
        async with asyncio.timeout(_AGENT_TIMEOUT):
            result = await graph.run_traced(
                ...
                user_id=user_id,
                key_id=key_id,
                # 大厂式 span 命名：根 agent span 用该 agent 的配置可读名（ac.name，
                # 如"销售助手"）而非机器 UUID；通用对话（无绑定 agent）才回退到语义
                # 节点名 "chat"。Span 存展示名，trace/详情页直接可读，无需反查配置。
                node_id=(ac.name if ac is not None and getattr(ac, "name", None) else agent_id) or "chat",
            )
    except TimeoutError:
        ...
        # LLM API 失败（HTTPStatusError 402/401/429 等）绝不能从任务泄漏出去
        # （"Task exception was never retrieved"），也不能让 run 卡在 running。
        # 发出错误事件 + 标记 run 失败。
        ...
        # Postgres/sqlite checkpointer 持有打开的连接；在每条退出路径
        # （done、timeout、cancel、error）上关闭它，否则每次 run 泄漏一个。
    ...
    await update_run_result(
        run_id=run_id, pm_document=pm_document, code=code,
        review=review, approved=True, status="converged",
    )
    await _notify_session_changed(user_id, session_id)
    ...
    try:
        atts = await list_attachments_by_run(run_id)
        if atts:
            links = [f"[📥 {a.filename}](/api/attachments/{a.id})" for a in atts]
    ...
    try:
        ...
        await log_key_usage(key_id=key_id or effective_api_key, user_id=user_id, run_id=run_id,
            provider=provider, model=model_used, tokens_prompt=input_tokens, tokens_completion=output_tokens)
    except Exception:
    ...
    if input_tokens > 0 or output_tokens > 0:
        try:
            ...
            logger.warning("Failed to record token usage for run %s", run_id)
