"""动态团队图——基于 LangGraph 的多 agent 工作流执行。"""
...
from context.response_cache import get_response_cache
...
class DynamicTeamGraph:
    def __init__(
        ...
        # 透传给节点内成本记录：run/team/user/key 归属
        ...
        self.token_budget_manager = TokenBudgetManager(
            total_budget=max_tokens * 10,  # 整个工作流的预算
            safety_margin=0.1,
        )
    ...
    async def run(
        ...
        # 迭代轮次会循环经过入口节点，因此递归预算随 max_rounds 缩放，
        # 而非硬编码的 100。
