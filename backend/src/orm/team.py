"""CommandLogDB、AuditLogDB、AttachmentDB ORM 模型。"""
...
class AuditLogDB(Base):
    """管理审计日志 —— 记录管理类 CRUD 操作（无 session 外键）。

    ``before_snapshot``/``after_snapshot`` 保存前后状态的 JSON 载荷（字符串化），
    使审计视图能展示究竟改了什么。
    ``hash``/``prev_hash`` 构成防篡改链：每条记录的哈希由前一条哈希加自身内容派生，
    因此篡改任一行都会破坏链条，并可通过校验扫描检测出来。
    """
