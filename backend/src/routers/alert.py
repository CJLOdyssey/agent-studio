"""告警管理 API——规则、事件、通知与订阅。

读取对所有已认证用户开放；变更需要 ``admin`` 角色
（遗留模式直接短路为 admin）。指标类型对照实时注册表校验，
使新的指标处理器无需修改路由即可被接受（开闭原则）。
"""
...
async def ack_alert_event(event_id: str, request: Request) -> Any:
    """确认一个 firing/resolved 事件；对已确认事件幂等。"""
    try:
