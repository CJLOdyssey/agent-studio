...
async def import_skill(
    ...
    """从 Anthropic Agent Skills SKILL.md 目录导入技能（multipart）。

    文件按相对路径键控：``SKILL.md``（必需）、``scripts/*``、
    ``references/*``、``resources/*``（可选）。``SKILL.md`` 的 frontmatter 与
    ...
    try:
        ...
        if not name:
            # 若 frontmatter 缺少 name，回退到首个 markdown 标题
            import re
...
async def import_skill_text(req: SkillImportRequest) -> Any:
    """从 Anthropic Agent Skills SKILL.md 格式导入技能（文本兼容）。

    旧 ``/api/skills/import`` 的向后兼容别名：解析 YAML frontmatter
    （``name``、``description``、``allowed-tools``、``metadata.category``），
    并将 markdown 正文映射到 ``instructions``。
    """
    try:
        import yaml
