...
class TestAgentTestHandler:
    ...
    @patch("repository.agents.get_agent_config", new_callable=AsyncMock, return_value=None)
    async def test_agent_not_found(self, mock_get, client):
        resp = client.post("/api/agents/nonexistent/test")
        assert resp.status_code == 404

    @patch("repository.agents.get_agent_config", new_callable=AsyncMock)
    async def test_agent_test_no_api_key(self, mock_get, client):
        ...
        with patch("core.config.load_config") as mock_cfg, \
             patch("repository.keys.get_default_api_key", new_callable=AsyncMock) as mock_key:
    ...
    async def test_agent_test_exception(self, mock_get, client):
        ...
        with patch("core.config.load_config", side_effect=Exception("config error")):
