...
class TestDiscoverMcpTools:
    ...
    def _reset_discovery_cache(self):
        ...
        """
        import tasks.discovery_cache as pu
    ...
    async def test_discover_timeout(self):
        with patch("tasks.discovery_cache.stdio_client", side_effect=TimeoutError()):
            from tasks.discovery_cache import _discover_mcp_tools
    ...
    async def test_discover_success(self):
        ...
        with patch("tasks.discovery_cache.stdio_client", return_value=stdio_cm), \
             patch("tasks.discovery_cache.ClientSession", return_value=session_cm), \
             patch("tasks.discovery_cache.asyncio") as mock_asyncio:
    ...
    async def test_discover_cache_key_varies_with_config(self):
        ...
        with patch("tasks.discovery_cache.stdio_client", side_effect=[stdio_cm_a, stdio_cm_b]), \
             patch("tasks.discovery_cache.ClientSession", side_effect=[session_cm_a, session_cm_b]):
    ...
    async def test_discover_timeout_negative_ttl_reused(self):
        ...
        with patch(
            "tasks.discovery_cache.stdio_client",
