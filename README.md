...
- 操作日志记录
- 审计追踪
- 全链路追踪：请求 → 后端 → DB → LLM 调用，trace_id 贯穿

### 🔬 可观测性（Observability）
- **事件存储**：SQLite 本地存储，后台线程写入，不阻塞主线程
- **全链路追踪**：基于 `contextvars` 的 trace_id 传播，横跨 HTTP/DB/LLM
- **自诊断**：`GET /api/debug/health` 暴露写入错误、队列积压、磁盘空间
- **错误分析**：自动聚合 trace 内错误，给出根因建议
- **启动守卫**：进程启动前写 marker，异常崩溃可追溯
- **磁盘保护**：低于阈值自动停止写入，防止磁盘写满
- **零成本**：批量写入 + 环形队列，对性能无影响
