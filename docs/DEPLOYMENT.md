...
> 适用版本：2026-06 · 单机 Docker Compose + GitHub Actions 自动化部署
...
│    │                                                           │
│    └── Deploy                                                   │
│        ├── Job 1: Build & Push → ghcr.io (ubuntu-latest)       │
│        └── Job 2: Deploy → ECS (self-hosted runner)            │
│                                                                  │
│  ECS (阿里云)                                                   │
...
│        ├── celery_worker (AI Agent 异步任务)                    │
│        ├── postgres (pgvector/pg16)                             │
│        └── redis (7-alpine)                                     │
└──────────────────────────────────────────────────────────────────┘
```

**核心流程：** 一次 `git push main` → CI 质量门禁 → Docker 镜像构建 + 推送到 GHCR → ECS 拉取代码 + 本地构建 + 滚动重启。
...
| Job | 运行环境 | 内容 |
|---|---|---|
| `backend-test` | ubuntu-latest | pytest + coverage（Py 3.11 / 3.12 矩阵） |
...
- **步骤：**
  ...
  4. 构建并推送镜像到 `ghcr.io/CJLOdyssey/virtual-software-team`：
     - `:latest` tag
...
- **依赖：** Job 1 完成
- **步骤：**
  1. **拉取代码：** 通过 SSH agent + 部署密钥，`git fetch --depth=1 origin main` + `git reset --hard origin/main`
  2. **构建前端：** 在 ECS 上原生执行 `npm ci` + `npm run build`
  3. **复制 .env：** 从固定路径复制环境变量文件
  4. **构建 + 重启：**
     ```bash
     docker compose -p 7 build api celery_worker   # 构建后端镜像
     docker compose -p 7 pull                       # 拉取外部镜像（postgres/redis/nginx）
     docker compose -p 7 up -d --remove-orphans     # 启动/重启所有服务
     docker image prune -f                          # 清理旧镜像
     ```
  5. **通知：** 通过 Webhook 发送部署结果（支持飞书/钉钉等 markdown 消息）

**并发控制：** `concurrency: group: deploy` + `cancel-in-progress: false`，保证同一时间只有一个部署在进行，不会中断正在进行的部署。
...
手动脚本流程：`git pull` → 前端构建 → `docker compose pull` → `docker compose up -d` → 清理旧镜像。
...
| 现象 | 可能原因 | 处理 |
|---|---|---|
| `docker compose build` 失败 | ECS 磁盘空间不足 | `docker system prune -f` |
| `npm ci` 失败 | 网络问题或 node 版本 | 手动 SSH 登录 ECS，检查 nvm 配置 |
| `git fetch` 失败 | SSH 密钥失效 | 检查 `~/.ssh/id_github` 是否有效 |
| 容器启动后立即退出 | 环境变量缺失 | 检查 `.env` 文件是否存在且完整 |
| API 健康检查失败 | 数据库未就绪 | `docker compose logs postgres` 查看 |

### 回滚部署

当前方案不支持镜像版本回滚（ECS 上每次重新构建）。
