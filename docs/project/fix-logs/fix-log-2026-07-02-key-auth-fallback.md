...
- 模块：`virtual_team/repository/keys.py`
...
| 文件 | 改动 |
|------|------|
| `virtual_team/repository/keys.py` | 4 个函数加 anonymous fallback：`get_api_key_for_use` 行首 fallback 查询、`get_default_api_key` 提取 `_resolve_key_row` 辅助函数、`delete_api_key` 加 owner_match/anonymous_fallback 判断、`update_api_key` 同上 |
