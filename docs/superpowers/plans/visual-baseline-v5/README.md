...
1. **light 全量重截验证（2026-08-04 fix 提交）**：评审发现初版 light/01-08、11 实为深色主题副本（与 dark 同名逐像素差异 <2.3%），本次已逐张重截全部 15 张：每张截图前用 evaluate_script 断言主题（`<html>` 无 dark class、body 背景 `rgb(255,255,255)`），截图后用像素抽查复核（light 占比 + 与 dark 同名文件 diff，弹窗类截图 dark%=0%）。12/02 均等侧栏会话列表与消息流渲染完成后再截。重截后 light 各张与 dark 同名差异均 ≥92%。**另核实：评审所称「light/12 为空白加载页」系误报**——已提交的 light/12 经像素分析含侧栏与消息文本（light%=97%、dark%=0%、与 dark/12 差异 100%），本次重截与其字节级一致（文件未变更）。light/02 与 light/12 内容相同（同会话消息流），二者仅 0.6% 像素差异。
2. **dark 02 与 dark 12 两图字节级相同**（同一会话「antd v6 迁移准备」的消息流），比对时按同图处理。
