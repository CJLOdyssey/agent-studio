...
- **镜像：** `crpi-j0fhvkobexa3ilkn.cn-shenzhen.personal.cr.aliyuncs.com/${ACR_NAMESPACE}/virtual-team:latest`
- **端口：** `8080:8080`
...
- **步骤：**
  ...
  4. 构建并推送镜像到 `crpi-j0fhvkobexa3ilkn.cn-shenzhen.personal.cr.aliyuncs.com/${{ secrets.ACR_NAMESPACE }}/virtual-team`：
     - `:latest` tag
...
# 4. 登录 ACR（首次部署需要）
docker login crpi-j0fhvkobexa3ilkn.cn-shenzhen.personal.cr.aliyuncs.com -u <ACR_USERNAME>
