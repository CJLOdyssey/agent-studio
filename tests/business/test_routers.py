...
def _make_agent(**kw):
    ...
    a.output_constraints = kw.get("output_constraints")
    a.tools = kw.get("tools")
    a.mcp = kw.get("mcp")
    a.skills = kw.get("skills")
    a.model = kw.get("model")
    a.temperature = kw.get("temperature")
...
def _mock_binding(**kw):
    ...
    b.tool_id = kw.get("tool_id")
    b.mcp_id = kw.get("mcp_id")
    b.skill_id = kw.get("skill_id")
    b.config_override = kw.get("config_override")
    b.tool_filter = kw.get("tool_filter")
    return b
...
def _mock_prompt(**kw):
    ...
    p.change_reason = kw.get("change_reason")
    p.is_active = kw.get("is_active", True)
    p.created_at = kw.get("created_at")
    return p
...
def _mock_schema(**kw):
    ...
    s.example = kw.get("example")
    s.created_at = kw.get("created_at")
    return s
...
class TestAddAgent:
    async def _call(self, **data):
        from virtual_team.routers.agents import AgentCreateRequest, add_agent
...
class TestEditAgent:
    async def _call(self, agent_id, **data):
        from virtual_team.routers.agents import AgentUpdateRequest, edit_agent
...
class TestCreatePrompt:
    async def test_success(self):
        ...
        with patch("virtual_team.routers.prompts.create_prompt", new_callable=AsyncMock, return_value=p):
            from virtual_team.routers.prompts import PromptCreateRequest, create_agent_prompt
...
class TestActivatePrompt:
    async def test_success(self):
        ...
        with patch("virtual_team.routers.prompts.activate_prompt", new_callable=AsyncMock, return_value=p):
            from virtual_team.routers.prompts import PromptActivateRequest, activate_agent_prompt
...
class TestCreateSchema:
    async def test_success(self):
        ...
        with patch("virtual_team.routers.schemas.create_output_schema", new_callable=AsyncMock, return_value=s):
            from virtual_team.routers.schemas import SchemaCreateRequest, create_agent_schema
...
class TestUpdateSchema:
    async def test_success(self):
        ...
        with patch("virtual_team.routers.schemas.update_output_schema", new_callable=AsyncMock, return_value=s):
            from virtual_team.routers.schemas import SchemaUpdateRequest, update_agent_schema
...
class TestBindTool:
    async def test_success(self):
        ...
        with patch("virtual_team.routers.bindings.bind_tool", new_callable=AsyncMock, return_value=b):
            from virtual_team.routers.bindings import BindToolRequest, bind_agent_tool
...
class TestBindMCP:
    async def test_success(self):
        ...
        with patch("virtual_team.routers.bindings.bind_mcp", new_callable=AsyncMock, return_value=b):
            from virtual_team.routers.bindings import BindMcpRequest, bind_agent_mcp
...
class TestBindSkill:
    async def test_success(self):
        ...
        with patch("virtual_team.routers.bindings.bind_skill", new_callable=AsyncMock, return_value=b):
            from virtual_team.routers.bindings import BindSkillRequest, bind_agent_skill
