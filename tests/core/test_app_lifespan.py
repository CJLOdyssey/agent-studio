...
class TestAppLifespan:
    def test_mask_url_with_credentials(self):
        from backend.core.app_lifespan import _mask_url
    ...
    def test_startup_report_includes_basic_info(self):
        from backend.core.app_lifespan import _startup_report
    ...
    async def test_check_redis_success(self):
        from backend.core.app_lifespan import _check_redis
    ...
    async def test_init_database_handles_exception(self):
        from backend.core.app_lifespan import _init_database

        with patch("backend.core.app_lifespan._do_init_db", side_effect=Exception("DB error")):
    ...
    async def test_shutdown_cancels_gc_task(self):
        ...
        from backend.core.app_lifespan import shutdown
    ...
    async def test_startup_calls_config_and_init(self):
        from backend.core.app_lifespan import startup

        mock_app = MagicMock()
        mock_app.state = MagicMock()

        with patch("backend.core.app_lifespan.load_config"):
            with patch("backend.core.app_lifespan._init_database", new_callable=AsyncMock):
                with patch("backend.core.app_lifespan._check_redis", new_callable=AsyncMock):
                    with patch("backend.core.app_lifespan.mark_started"):
    ...
    async def test_startup_handles_exception(self):
        ...
        with patch("backend.core.app_lifespan.load_config"):
            with patch("backend.core.app_lifespan._init_database", side_effect=RuntimeError("Fatal")):
                with patch("backend.core.app_lifespan.record_crash") as mock_record:
                    with pytest.raises(RuntimeError, match="Fatal"):
                        await startup(mock_app)
                    mock_record.assert_called_once()

    def test_env_helper(self):
        from backend.core.app_lifespan import _env

        with patch("backend.core.app_lifespan.os.environ", {"MY_KEY": "my_value"}):
