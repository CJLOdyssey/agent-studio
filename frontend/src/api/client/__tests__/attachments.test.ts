...
import { uploadAttachment, uploadAttachments, deleteAttachment } from '../attachments';
...
describe('uploadAttachment', { tags: ['unit'] }, () => {
  ...
  it('reports upload progress', async () => {
    mockApi.post.mockImplementation(async (_url: string, _form: FormData, config: { onUploadProgress?: (e: { loaded: number; total: number }) => void }) => {
