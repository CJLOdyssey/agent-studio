...
export default function ToggleSwitch({ checked, onChange, size = 'md', label }: Props) {
  return (
    ...
    >
      ...
      <span
        className={
          `absolute inset-0 rounded-[22px] transition-colors duration-200 bg-[var(--da-bg-hover)] peer-checked:bg-[var(--da-accent-indigo)]
          before:content-[''] before:absolute before:rounded-full before:transition-all before:duration-200 before:bg-[var(--da-text-muted)] peer-checked:before:bg-[var(--da-text-on-accent)]
          ${size === 'sm'
