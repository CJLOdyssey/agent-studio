...
export default function SettingsModal({ onClose }: Props) {
  ...
  return (
    <Modal title={t('settings.title')} onClose={onClose} className="settings-modal"
      footer={
        <>
          <button className="btn btn-secondary" onClick={onClose}>{t('settings.cancel')}</button>
          <button className="btn btn-primary" onClick={onClose}><Save size={14} />{t('settings.save')}</button>
        </>
      }
    >
      <div className="settings-body">
        <div className="settings-sidebar">
          {([['general', Globe], ['editor', Code], ['shortcuts', Keyboard], ['account', User], ['notifications', Bell]] as const).map(([tab, Icon]) => (
            <button key={tab} className={`settings-tab ${activeTab === tab ? 'active' : ''}`} onClick={() => setActiveTab(tab as SettingsTab)}>
              <Icon size={16} /><span>{t('settings.' + tab)}</span>
            </button>
          ))}
        </div>
        ...
        <div className="settings-content">
          {activeTab === 'general' && (
            <div className="settings-section">
              ...
              <div className="settings-item">
                ...
                <select value={settings.theme} onChange={e => updateSettings({ theme: e.target.value as 'dark' | 'light' | 'system' })} className="settings-select">
              ...
              <div className="settings-item">
                ...
                <div className="settings-range-wrapper">
                  ...
                  <input ref={rangeRef} type="range" min={12} max={16} step={1} value={settings.fontSize}
                    onChange={e => updateSettings({ fontSize: Number(e.target.value) })}
                    onInput={e => { e.currentTarget.style.setProperty('--pct', ((+e.currentTarget.value - 12) / 4 * 100) + '%'); }}
              ...
              <div className="settings-item">
                ...
                <select value={settings.sendMode} onChange={e => updateSettings({ sendMode: e.target.value as 'enter' | 'ctrl-enter' })} className="settings-select">
          ...
          {activeTab === 'editor' && (
            <div className="settings-section">
              ...
              <div className="settings-item">
                ...
                <select value={settings.editorFontSize} onChange={e => updateSettings({ editorFontSize: Number(e.target.value) })} className="settings-select">
                  {[11, 12, 13, 14, 15, 16, 18, 20].map(v => <option key={v} value={v}>{v}px</option>)}
                </select>
              </div>
              <div className="settings-item">
                <div className="settings-item-info">
                ...
                <select value={settings.tabSize} onChange={e => updateSettings({ tabSize: Number(e.target.value) })} className="settings-select">
          ...
          {activeTab === 'shortcuts' && (
            <div className="settings-section">
              ...
              <p className="settings-item-desc" style={{ marginBottom: 16 }}>{t('settings.shortcutsDesc')}</p>
              {shortcuts.map(s => (
                <div key={s.keys} className="settings-item">
                  <span style={{ fontSize: 'var(--da-font-size-sm)', color: 'var(--da-text-primary)' }}>{t(s.descKey)}</span>
          ...
          {activeTab === 'account' && (
            <div className="settings-section">
              ...
              <div className="settings-profile">
                <div className="settings-avatar"><User size={32} /></div>
              ...
              <div className="settings-item">
                <div className="settings-item-info">
                  <label>{t('settings.username')}</label><span className="settings-item-desc">{t('settings.usernameDesc')}</span>
                </div>
                <input type="text" value={username} onChange={e => setUsername(e.target.value)} className="settings-input" />
              </div>
              <div className="settings-item">
                <div className="settings-item-info">
                  <label>{t('settings.email')}</label><span className="settings-item-desc">{t('settings.emailDesc')}</span>
                </div>
              ...
              <div className="settings-item">
                ...
                <button className="btn btn-sm btn-secondary" onClick={() => {}}>{t('settings.changePassword')}</button>
              </div>
            </div>
          )}
            ...
            <div className="settings-section">
              ...
              {[
                ...
                <div className="settings-item" key={label}>
                  <div className="settings-item-info"><label>{label}</label><span className="settings-item-desc">{desc}</span></div>
              ...
              <div className="settings-item">
                <div className="settings-item-info">
                  <label>{t('settings.emailNotif')}</label><span className="settings-item-desc">{t('settings.emailNotifDesc')}</span>
                </div>
