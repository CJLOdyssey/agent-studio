...
const ConversationsList = memo(function ConversationsList({
  ...
  const renderConversationItem = (conv: Conversation) => {
    ...
    return (
      <div
        key={conv.id}
        className={`agentstudio-conv-item group flex items-center justify-between py-2 px-2 rounded-md cursor-pointer transition-colors duration-150 gap-2 hover:bg-[var(--da-bg-hover)] ${isActive ? 'bg-[var(--da-bg-elevated)]' : ''}`}
      ...
      >
        <div className="agentstudio-conv-item-content flex-1 min-w-0">
          <div className="agentstudio-conv-item-title text-sm text-[var(--da-text-primary)] leading-[1.3] overflow-hidden text-ellipsis whitespace-nowrap flex items-center gap-1">
            {isTeam && (
              <span className="agentstudio-conv-item-agent-icon flex-shrink-0 flex items-center" style={{ color: 'var(--da-accent)' }}>
                <Users size={12} />
              </span>
            )}
          ...
          <div className="agentstudio-conv-item-meta text-xs text-[var(--da-text-muted)] mt-[2px] flex items-center gap-1">
            {isTeam && (
              <span className="agentstudio-conv-item-agent-name text-[var(--da-text-secondary)] font-medium" style={{ color: 'var(--da-accent)' }}>{conv.teamName || '团队'}</span>
            )}
            {agent && !isTeam && (
              <span className="agentstudio-conv-item-agent-name text-[var(--da-text-secondary)] font-medium">{agent.name}</span>
            )}
        ...
        <button
          className="agentstudio-conv-delete flex-shrink-0 p-1 rounded bg-transparent border-none text-[var(--da-text-muted)] cursor-pointer opacity-0 transition-opacity flex items-center justify-center group-hover:opacity-100 hover:text-[var(--icon-status-error)]"
          onClick={(e) => {
            e.stopPropagation();
            onDelete(conv.id);
          }}
    ...
    <div className="agentstudio-conversations-list flex flex-col gap-2 mb-1">
      <Virtuoso
        ...
        itemContent={(_index: number, item: (typeof flatItems)[number]) =>
          item.type === 'group' ? (
            <div className="agentstudio-conv-group-label text-xs font-semibold text-[var(--da-text-muted)] py-1 px-2 flex items-center gap-1">{item.label}</div>
          ) : (
