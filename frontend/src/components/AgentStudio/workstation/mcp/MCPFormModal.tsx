...
import { MCP_TYPE_OPTIONS, MCP_TYPE_LABEL } from './mcp.constants';
    ...
    >
      ...
      <div className="flex items-start gap-4">
        ...
        <label className="flex items-center gap-2 pt-5 cursor-pointer select-none">
          <input type="checkbox" className="w-4 h-4 accent-[var(--color-accent)] cursor-pointer" checked={formData.enabled ?? true} onChange={(e) => setFormData((f) => ({ ...f, enabled: e.target.checked }))} aria-label={t('mcp.form_enabled')} />
          <span className="text-xs font-medium text-[var(--color-text-secondary)]">{t('mcp.form_enabled')}</span>
        </label>
      </div>
      {formData.type === 'stdio' ? (
      ...
      <div className="flex flex-col gap-1">
        <label className="text-xs font-medium text-[var(--color-text-secondary)]">{t('mcp.form_version')}</label>
