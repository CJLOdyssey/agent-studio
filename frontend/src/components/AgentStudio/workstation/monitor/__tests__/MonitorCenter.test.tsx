...
vi.mock('../LLMTraces', () => ({ default: () => <div data-testid="llm-traces" /> }));
...
describe('MonitorCenter', { tags: ['integration'] }, () => {
  ...
  it('switches to the alert tab and renders alert panels', async () => {
    ...
    expect(screen.getByTestId('slo-budget')).toBeInTheDocument();
  });
  ...
  it('switches to the traces tab', async () => {
    ...
    expect(await screen.findByTestId('llm-traces')).toBeInTheDocument();
  });
});
