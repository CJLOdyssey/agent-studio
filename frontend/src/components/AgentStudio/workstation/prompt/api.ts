...
function toEntry(item: { id: string; name: string; description?: string | null; category: string; content: string; model: string | null; status: string; version: string; created_at: string }): PromptEntry {
  return {
    id: item.id,
    name: item.name,
...
const { bind: promptAPI, setAPI: setPromptAPI } = defineCrudModule<PromptEntry, PromptFormData>({
  ...
  create: async (data) => { const item = await createPrompt({ name: data.name, description: data.description, category: data.category, content: data.content, status: data.status }); return toEntry(item); },
