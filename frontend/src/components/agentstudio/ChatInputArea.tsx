...
const ChatInputArea = memo(
  forwardRef<InputToolbarHandle, ChatInputAreaProps>(function ChatInputArea(
    ...
    useEffect(() => {
      const handler = () => {
        try {
          const stored = localStorage.getItem('agentstudio-selected-model');
          if (stored) setSelectedModel(stored);
        } catch { /* non-fatal */ }
      };
      window.addEventListener('agentstudio-model-changed', handler);
      return () => window.removeEventListener('agentstudio-model-changed', handler);
    }, []);
