...
/**
 * 会话列表管理器——追踪 localStorage 中已保存的会话。
 *
 * 该 hook 仅处理会话元数据（标题、时间戳、持久化的消息列表）。
 * 真正的消息状态由 chatStore（Zustand）管理，它是实时 API/WebSocket 消息的唯一真源。
 *
 * 已移除 mock 兜底响应——遗留的 mock 系统见 utils/agentResponses.ts，
 * 现仅在未配置任何 API agent 时使用。
 */
export function useConversation() {
  ...
  // 立即将会话持久化到 localStorage（不只依赖防抖 effect）。
  ...
  // 会话变化时持久化到 localStorage（temp 占位不落盘）
  useEffect(() => {
    try {
      localStorage.setItem(
        'agentstudio-conversations',
      ...
      // localStorage 满或不可用——非致命
    }
  }, [conversations]);

  // 监听外部写入（来自 chatStore 同步）
  useEffect(() => {
    const handler = () => {
      try {
        const saved = localStorage.getItem('agentstudio-conversations');
  ...
  // 会话归属于用户：仅在认证建立后才拉取，
  // 并在认证状态翻转（挂载后登录/刷新完成）时重新拉取。
  // 与 ragbase 的 useQuery(enabled: isAuthenticated) 语义一致。
  useEffect(() => {
    ...
    listSessions(100).then((sessions) => {
      ...
      setConversations((prev) => {
        ...
        // 深度比较：检查 session ID 集合是否变化（而非仅比较长度）
  ...
  /** 按 ID 删除会话——同时移除本地记录和服务端 session。 */
  ...
  /** 置顶/取消置顶会话——乐观更新 localStorage + 服务端同步。 */
